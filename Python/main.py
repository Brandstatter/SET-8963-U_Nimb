...
async def slash_race(ctx,
    race_option : discord.Option(str, choices = ['Humano', 'Anão', 'Dahllan', 'Elfo', 'Goblin', 'Lefou', 'Minotauro', 'Qareen', 'Golem', 'Hynne', 'Kliren', 'Medusa', 'Osteon', 'Sereia/Tritão', 'Sílfide', 'Suraggel', 'Trog', 'Eiradaan', 'Galokk', 'Meio-Elfo', 'Sátiro'])
):
...
async def slash_origins(ctx,
    origem: Option(str, "Pick a color!", autocomplete=autocomplete_origin)                 
                        ):
    id = await Commands.origins.slash_search(ctx, origem)
...
async def slash_conditions(ctx, 
    ...
    id = await Commands.conditions.slash_search(ctx, condition_option)
    embed = await Commands.conditions.embed_condition(ctx, id)
    await ctx.respond(embed = embed)
